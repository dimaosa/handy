...
class FilterImage : NSObject, UIScrollViewDelegate{
    ...
    init(imgView: UIImageView, image: UIImage, phoneHand: PhoneSkin){
        super.init()
        filterImageView = UIImageView(frame: CGRectMake(0,0,imgView.bounds.width, imgView.bounds.height))
        originalImage = image
        phoneSkin = phoneHand
        //Background blur imageView with originalPhoto
        //let bcg = blurBackground
        let blurBackgroun = blurBackground
        filterImageView.addSubview(blurBackgroun)
    ...
    //MARK! - Image creation
    internal var blurBackground: UIImageView {
